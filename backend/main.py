...
BTC5m: 7-indicator weighted TA (window delta dominant)
      Entry T-30s poll loop → fire at best signal → T-5s hard deadline
...
async def btc5m_loop():
    """Dedicated BTC5m loop — polls every 8s, enters snipe zone T-30s to T-5s"""
    print(f"[BTC5m] loop started")
    b5 = S.btc5m
        ...
        while True:
            try:
                ...
                if now_ts - b5["last_kline_fetch"] >= 55 or not b5["klines"]:
                    ...
                    if kl:
                        b5["klines"] = kl
                        # window open = close of candle just before window_ts
                ...
                # Entry zone: T-30s to T-5s
                b5["secs_left"]     = secs_left
                b5["in_entry_zone"] = 5 <= secs_left <= 30

                # Fire if: in zone, not fired, has direction, running, not gas-paused
                if (b5["in_entry_zone"] and not b5["entry_fired"]
                        and sig["dir"] and sig["confidence"] >= 0.25
                        and S.running and not S.gas_paused):
...
async def fetch_soccer_markets(sess) -> list:
    ...
    try:
        ...
        for m in unique:
            ...
            if rs > 86400 * 3: continue   # skip > 3 days
            m["_resolve_sec"] = rs
            soccer.append(m)
...
def risk_ok(mid: str, sig: dict) -> tuple[bool, str]:
    if not S.running:           return False, "Bot tidak berjalan"
    if S.gas_paused:            return False, f"Gas stop: {gas_orders_left()} order tersisa"
    if S.daily_pnl <= -C.daily_loss_limit: return False, f"Daily loss limit ${C.daily_loss_limit}"
    ops = [p for p in S.positions if p["status"]=="open"]
    if len(ops) >= C.max_open_pos: return False, f"Max {C.max_open_pos} posisi"
    if any(p["market_id"]==mid for p in ops): return False, "Sudah ada posisi"
    if S.capital < C.min_bet:   return False, f"Capital ${S.capital:.2f} < min ${C.min_bet}"
    return True, "OK"
...
async def open_position(market: dict, sig: dict):
    async with _lock:
        ...
        if not ok:
            ...
            add_log("REJECTED",{"reason":reason,"question":market["question"][:45],"strategy":sig.get("strategy","")})
            return
